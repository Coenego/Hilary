    ...
    PrincipalsDAO.getPrincipal(ticket.createdBy, function(err, user) {
        ...
        PublicationsAPI.getPublication(ctx, ticket.publicationId, function(err, publication) {
            ...
            _getOrCreateZendeskUser(client, user.displayName, userEmail, function(err, zenDeskUser) {
                ...
                // The publication funders
                var funders = (publication.funders) ? [_formatFunders(publication.funders), _getOtherFunders(publication.funders)].join('; ') : null;
                var institution = publication.institution || 'University of Cambridge';

                // The ZenDesk ticket
                var zenDeskTicket = {
                    'ticket': {
                        'custom_fields': [
                            {'id': ZenDeskConfig.getValue(tenantAlias, 'fields', 'externalId'), 'value': ticket.externalId},
                            {'id': ZenDeskConfig.getValue(tenantAlias, 'fields', 'downloadLink'), 'value': downloadURL},
                            {'id': ZenDeskConfig.getValue(tenantAlias, 'fields', 'publicationTitle'), 'value': publication.displayName},
                            {'id': ZenDeskConfig.getValue(tenantAlias, 'fields', 'department'), 'value': publication.department},
                            {'id': ZenDeskConfig.getValue(tenantAlias, 'fields', 'correspondingAuthor'), 'value': publication.authors[0]},
                            {'id': ZenDeskConfig.getValue(tenantAlias, 'fields', 'correspondingAuthorInstitution'), 'value': publication.institution},
                            {'id': ZenDeskConfig.getValue(tenantAlias, 'fields', 'journalName'), 'value': publication.journalName},
                            {'id': ZenDeskConfig.getValue(tenantAlias, 'fields', 'acceptanceDate'), 'value': acceptanceDate},
                            {'id': ZenDeskConfig.getValue(tenantAlias, 'fields', 'funders'), 'value': funders}
                        ],
                ...
                client.tickets.create(zenDeskTicket, function(err, req, result) {
                    ...
                    // Create a private ticket comment that contains some data the user should not see
                    var zenDeskComment = {
                        'ticket': {
                            'comment': {
                    ...
                    // Add the private comment to the created ticket
...
/**
 * Adds a leading zero to be compliant with the ZenDesk format
 *
...
var _addLeadingZero = function(value) {
    ...
    return util.format('0%d', value);
};

/**
 * Format a millisecond timestamp for use in the ZenDesk ticket description.
 *
...
var _formatAcceptanceDate = function(dateMillis) {
    ...
    var month = _addLeadingZero(date.getMonth() + 1);
    var day = _addLeadingZero(date.getDate());

    return util.format('%d-%s-%s', year, month, day);
};
