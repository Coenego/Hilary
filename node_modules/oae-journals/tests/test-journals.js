...
describe('Journals', function() {
    ...
    describe('#importJournals()', function() {
        ...
        it('verify permissions', function(callback) {
            JournalsRestAPI.importJournals(anonymousCamRestContext, {'file': getFileStream}, function(err) {
                assert.strictEqual(err.code, 404);

                JournalsRestAPI.importJournals(camUserRestContext, {'file': getFileStream}, function(err) {
                    assert.strictEqual(err.code, 404);

                    JournalsRestAPI.importJournals(camAdminRestContext, {'file': getFileStream}, function(err) {
                        assert.strictEqual(err.code, 404);

                        JournalsRestAPI.importJournals(anonymousGlobalRestContext, {'file': getFileStream}, function(err) {
                            assert.strictEqual(err.code, 401);

                            JournalsRestAPI.importJournals(globalAdminRestContext, {'file': getFileStream}, function(err) {
                                assert.ok(!err);
                                return callback();
                            });
                        });
